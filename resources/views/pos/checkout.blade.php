...
<div class="grid gap-6 lg:grid-cols-5">
            ...
            @foreach($products as $product)
                <div class="product-card" data-name="{{ strtolower($product->name) }}" data-sku="{{ strtolower($product->sku ?? '') }}">
                    <button type="button" onclick="addToCart({{ json_encode($product) }})"
                            class="pos-product group w-full rounded-xl border border-gray-100 bg-white p-4 text-left shadow-sm transition hover:border-indigo-200 hover:shadow-md active:scale-[0.98] dark:border-gray-800 dark:bg-gray-900 dark:hover:border-indigo-800">
                        <p class="text-sm font-medium text-gray-900 group-hover:text-indigo-600 dark:text-white dark:group-hover:text-indigo-400">{{ $product->name }}</p>
                        <p class="mt-1 text-lg font-semibold text-indigo-600 dark:text-indigo-400">{{ number_format($product->price, 2) }}</p>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Stock: {{ $product->stock_quantity }} {{ $product->measurement_unit }}</p>
                    </button>
                </div>
            @endforeach
        </div>
    ...
    <div class="lg:col-span-2">
        <x-card :padding="false" class="sticky top-20 overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-100 bg-indigo-600 px-5 py-3 dark:border-gray-800">
            ...
            <div id="cartItems" class="max-h-[40vh] space-y-0 overflow-y-auto px-5 py-3">
                <p class="text-sm text-gray-500 dark:text-gray-400">Tap products to add</p>
            </div>

            <div class="space-y-4 border-t border-gray-100 p-5 dark:border-gray-800">
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-500 dark:text-gray-400">Total</span>
                    <span id="cartTotal" class="text-xl font-semibold text-gray-900 dark:text-white">0.00</span>
                </div>
                ...
                <div id="customerSelectWrap" class="hidden">
                    <x-select id="customerId" label="Credit Customer">
                        ...
                        @foreach($customers as $c)
                            <option value="{{ $c->id }}">{{ $c->name }} (Bal: {{ number_format($c->outstanding_balance, 2) }})</option>
                        @endforeach
                    </x-select>
                </div>
...
function renderCart() {
    ...
    if (!cart.length) {
        wrap.innerHTML = '<p class="text-sm text-gray-500 dark:text-gray-400">Tap products to add</p>';
        return;
    }

    wrap.innerHTML = cart.map((item, idx) => `
        <div class="border-b border-gray-100 py-3 last:border-0 dark:border-gray-800">
            <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex shrink-0 gap-1">
                    <button type="button" onclick="changeQty(${idx}, -1)" class="rounded-lg border border-gray-300 px-2 py-1 text-sm shadow-sm hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800">−</button>
                    <button type="button" onclick="changeQty(${idx}, 1)" class="rounded-lg border border-gray-300 px-2 py-1 text-sm shadow-sm hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800">+</button>
                    <button type="button" onclick="removeItem(${idx})" class="rounded-lg border border-red-200 px-2 py-1 text-sm text-red-600 shadow-sm hover:bg-red-50 dark:border-red-900 dark:hover:bg-red-950">×</button>
                </div>
            </div>
        </div>
    `).join('');
