...
@php
    ...
    $batchModeEnabled = $batchModeEnabled ?? false;
@endphp

<x-page-header :title="$product->name" :subtitle="$batchModeEnabled ? 'Product details and batch inventory' : 'Product details and stock levels'">
    <x-slot name="actions">
        @can('update', $product)
            <x-button variant="secondary" size="sm" href="{{ tenant_route('tenant.inventory.edit', ['product' => $product]) }}">Edit Product</x-button>
            @if($batchModeEnabled && ! $isVariable)
...
<x-card class="mb-6">
    <dl class="grid gap-4 sm:grid-cols-2">
        ...
        @if(! $isVariable)
            <div>
                <dt class="text-xs font-medium uppercase tracking-wide text-gray-500">Stock on hand</dt>
