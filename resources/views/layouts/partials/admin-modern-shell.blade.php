@php
    ...
    $lowStockCount = 0;
    $lowStockDropdownItems = collect();
    if ($business && $user->can('view-inventory')) {
        $lowStockService = app(\App\Services\LowStockAlertService::class);
        $lowStockCount = min(99, $lowStockService->count($business));
        $lowStockDropdownItems = $lowStockService->lowStockProducts($business, $user, 8);
    }
        ...
        <header class="sticky top-0 z-30 border-b border-gray-200/80 bg-white/95 px-4 py-3 backdrop-blur-sm sm:px-6 lg:px-8">
            <div class="flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    ...
                    @can('view-inventory')
                        @include('layouts.partials.low-stock-notifications-dropdown')
                    @endcan
...
(function () {
    ...
    var stockToggle = document.getElementById('low-stock-notifications-toggle');
    var stockPanel = document.getElementById('low-stock-notifications-panel');
    var stockRoot = document.getElementById('low-stock-notifications');

    function closeStockPanel() {
        if (!stockPanel || !stockToggle) return;
        stockPanel.classList.add('hidden');
        stockToggle.setAttribute('aria-expanded', 'false');
    }

    stockToggle?.addEventListener('click', function (event) {
        event.stopPropagation();
        if (!stockPanel) return;
        var open = stockPanel.classList.contains('hidden');
        if (open) {
    ...
    document.addEventListener('click', function (event) {
        if (!stockRoot || !stockPanel || stockPanel.classList.contains('hidden')) return;
        if (!stockRoot.contains(event.target)) {
            closeStockPanel();
        }
    });
