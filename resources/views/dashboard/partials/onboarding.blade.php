...
<div class="mb-8 flex min-h-[50vh] flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white px-6 py-16 text-center shadow-sm sm:min-h-[55vh] sm:px-12">
    ...
    @if($onboarding['needs_employees'])
        ...
        <form method="POST" action="{{ tenant_route('tenant.onboarding.sole-proprietor') }}" class="mt-4">
            @csrf
            <button type="submit"
                    class="inline-flex items-center justify-center rounded-lg border border-amber-300 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-900 shadow-sm transition hover:bg-amber-100">
                I'm running the business alone — I'll act as cashier
            </button>
        </form>
    @endif
</div>
